...
export default function Three() {
    ...
    useEffect(() => {
        ...
        const choice2 = operators2[operation];
        seta(() => a);
        setb(() => b);
        setc(() => c);
        setOp(() => choice);
        setOp2(() => choice2);
        const question = `${a}${choice}${b}${choice2}${c}`
    ...
    function handleClick() {
        setClick(click + 1);
        // (val == output) ? alert("right") : alert("wrong");
    ...
    return (
        <div className="flex flex-col lg:flex-row w-full min-h-screen">
            <div className='lg:w-1/2 bg-[url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTHY3ZxDNlaDhF77b6wHVcCVWQJGqBpwS4fAgr0MwkF0VTN2Gn4YBEDj7oL&s=10)] w-full bg-cover bg-center text-white  h-full lg:h-screen flex justify-center items-center '>
                <h1 className='text-2xl   p-3 ml-3'>
                    <span className='font-bold font-sans'>Instructions</span> <br />
                    You will be given three  2 or 3-digit numbers. <br />
            ...
            <div className=" text-white flex flex-col flex-wrap w-full lg:w-1/2 min-h-screen lg:h-screen justify-center items-center border-2 border-amber-300 bg-black p-4">
                <div className='flex flex-col flex-wrap justify-center items-center border-2 border-amber-300 rounded lg:w-100  lg:h-64 sm:w-80 sm:h-80 gap-y-6 w-70 h-64 sm:gap-y-10'>
                    <div className='flex flex-wrap bg-sky-400 p-2 rounded items-center text-xl sm:text-2xl gap-4 sm:gap-x-10 lg:gap-x-10 '>
                        <h1 >Question : </h1>
                        <p >{a} {op} {b} {op2} {c}</p>
                    </div>
                    <div className='flex flex-col lg:w-60 lg:h-25 w-60 h-20 p-2 bg-blue-600 gap-2 lg:gap-4 rounded'>
                        <input
                            value={val}
